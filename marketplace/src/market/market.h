...
class Market{
    ...
    public:
        ...
        bool add_seller(const Seller &seller);
        inline int n_clients() const {return client_list_.size();}
        inline int n_sellers() const {return seller_list_.size();}
        bool delete_client(const Client &client);
        bool delete_seller(const Seller &seller);
        bool add_product_client(const Product &p, std::string id_client);
        bool add_product_seller(const Product &p, std::string id_seller);
        bool delete_product_client(const Product &p, std::string id_client);
        bool delete_product_seller(const Product &p, std::string id_seller);
